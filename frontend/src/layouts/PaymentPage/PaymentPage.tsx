...
export const PaymentPage = () => {
  ...
  useEffect(() => {
    const fetchFees = async () => {
      if (authState && authState.isAuthenticated) {
        const url = `${process.env.REACT_APP_API}/payments/search/findPaymentByUserEmail?userEmail=${authState.accessToken?.claims.sub}`;
        const optionRequest = {
          method: "GET",
          headers: {
        ...
        const paymentResponse = await fetch(url, optionRequest);
  ...
  async function checkout() {
    ...
    const stripeResponse = await fetch(url, optionRequest);
      ...
      {fees > 0 && (
        <div className="card mt-3">
          ...
          <div className="card-body">
            <h5 className="card-title mb-3">Credit Crad</h5>
